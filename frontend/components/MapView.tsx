...
// ---------- District support ----------
const norm = (s: string) => s.toLowerCase().replace(/\s+/g, '').replace(/district$/, '');
type DistrictGeo = {
  type: 'FeatureCollection';
  features: Array<{ type: 'Feature'; properties: Record<string, any>; geometry: any }>;
};
const getFeatureName = (props: Record<string, any>): string =>
  props?.name ||
  props?.NAME ||
  props?.ADM2_EN ||
  props?.ADM2_NAME ||
...
// --- Clustered fire points layer ---
function MarkerClusterLayer({
  firePoints,
  onPointSelect,
}: {
  firePoints: FirePoint[];
  onPointSelect: (point: FirePoint) => void;
}) {
  const map = useMap();
